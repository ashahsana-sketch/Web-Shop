...
export default async function Home({ searchParams }: HomeProps) {
  ...
  // Single fetch call with Next.js Time-Based Revalidation (cached for 60s)
  const response = await fetch(
    `${API_URL}/products?_sort=id&_order=desc&_expand=category`,
    {
      next: { revalidate: 60, tags: ["products"] },
  ...
  const allProducts: Product[] = Array.isArray(response) ? response : [];

  // 1. Calculate overall summary metrics from the complete dataset
  const total = allProducts.length;
  ...
  // 2. Apply URL search filters (category and stock status) in memory
  ...
  // 3. Perform pagination on the filtered dataset
